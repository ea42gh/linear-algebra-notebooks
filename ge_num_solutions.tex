...
\tikzstyle{rect} = [draw, rectangle, text width=6em, text centered, minimum height=2em]
\tikzstyle{elli} = [draw, ellipse,                                  minimum height=2em]
\tikzstyle{diam} = [draw, diamond, text width=6em, text badly centered, minimum height=2em]
\tikzstyle{circ} = [draw, circle,  text width=1.5cm, minimum width=8pt, inner sep=10pt]
\tikzstyle{line} = [draw, -stealth, thick]

\begin{tikzpicture}[node distance = 1.5cm, auto, scale=0.5]
    \node [rect, rounded corners]                       (start) {$( R \mid \tilde{b} )$ } ;
    \node [diam, below of=start,        node distance = 2.5cm]  (q_zeros)          {any zero rows?} ;
    \node [diam, left of=q_zeros, xshift=-3cm,  yshift=-2.5cm]    (q_contradiction)  {Contradiction?} ;
    \node [diam, right of=q_contradiction, node distance = 8.5cm] (q_free_variables) {Free Variables?} ;

    \node [circ, below of=q_contradiction, node distance=4cm]  (no_solution)   {$\quad$ No Solution} ;
    \node [circ, below of=q_free_variables, node distance=4cm]       (one_solution)                    {Unique Solution} ;
    \node [circ, right of=one_solution, node distance=5cm] (inf_solutions) {$\quad\infty\quad$ Solutions} ;
